...
namespace RobotNav
{
    internal class GBFS<T> : Pathfinder<T>
    {
        PriorityQueue<State<T>, float> _frontier = new();

        /// <summary>
        /// A pathfinder that uses Greedy Depth-First Search
        /// </summary>
        public GBFS()
        {
        ...
        public override void AddArrayToFrontier(List<State<T>> nodes)
        {
            foreach (var node in nodes)
            {
                ...
                {
                    _frontier.Enqueue(node, node.HCost);
                    _discovered++;
                }
            }
        }
        ...
        /// <summary>
        /// Uses Greedy-Best-First Search to find a solution, returning an in-order list of steps to the goal
        /// </summary>
